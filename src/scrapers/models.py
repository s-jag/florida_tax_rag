...
class StatuteMetadata(BaseModel):
    ...
    effective_date: date | None = Field(default=None, description="Effective date of this version")
...
class TAAMetadata(BaseModel):
    ...
    issue_date: date | None = Field(default=None, description="Date issued")
    tax_type: str = Field(default="", description="Type of tax addressed (Sales, Corporate, etc.)")
    tax_type_code: str = Field(
        default="", description="Single letter tax type code (A, B, C, etc.)"
    )
...
class RawTAA(BaseModel):
    ...
    pdf_path: str | None = Field(default=None, description="Path to downloaded PDF")
    source_url: str = Field(..., description="URL where this was scraped from")
    scraped_at: datetime = Field(
        default_factory=lambda: datetime.now(),
        description="Timestamp of when this was scraped",
...
class CaseMetadata(BaseModel):
    ...
    citations: list[str] = Field(
        default_factory=list, description="Case citations (e.g., '215 So. 3d 46')"
    )
    ...
    date_filed: date | None = Field(default=None, description="Date the opinion was filed")
    ...
    statutes_cited: list[str] = Field(
        default_factory=list, description="Statute citations in opinion"
    )
    cases_cited: list[int] = Field(
        default_factory=list, description="CourtListener IDs of cited cases"
    )
...
class RawCase(BaseModel):
    ...
    opinion_html: str | None = Field(
        default=None, description="HTML version of opinion if available"
    )
    source_url: str = Field(..., description="CourtListener URL for this case")
    pdf_url: str | None = Field(default=None, description="URL to opinion PDF if available")
    scraped_at: datetime = Field(
        default_factory=lambda: datetime.now(),
        description="Timestamp of when this was scraped",
    )
...
class ScrapedDocument(BaseModel):
    ...
    statute: RawStatute | None = None
    rule: RawRule | None = None
    taa: RawTAA | None = None
    case: RawCase | None = None
